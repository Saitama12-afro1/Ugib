function dinamic_form(event) {
    ...
    if (this_choise == "APR")
    $.get( "/get_html_grr-stage/", data = {'choise':this_choise}, function( data ) {
        $( "#valid_div" ).html( data );
...
function current_height(area){
  ...
  function TSNIGRI_validation(choise){
    ...
    var pattern =/[А-Яа-яёЁ\-,\s]+_[0-9]+$/
    var pattern2 = /[А-Яа-яёЁ\-,\s]+_[0-9]+(_[0-9А-Яа-яёЁ]+)+/

    const error = document.querySelector(".error");
    if (pattern.test(stor_folder.value) || pattern2.test(stor_folder.value) ){
        error.textContent = "";
    ...
    function showError() {
        error.textContent = "Не соответствует шаблону Аллара-Сахская_98_2017"
    ...
    stor_desc.addEventListener("input", (event) =>{
        ...
        var pattern = /[0-9][0-9][0-9][0-9]_[0-9][0-9]_[0-9][0-9]\sПротокол\s[А-Яа-яёЁ0-9]+/
        const error = document.querySelector("#error_stor_desc");
        if (pattern.test(stor_desc.value)){
            error.textContent = "";
            error.className = "error";
        }else{
            showError()
